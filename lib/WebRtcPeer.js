  ...
  if (mode !== 'recvonly' && !videoStream && !audioStream) {
    function getMedia(constraints) {
      ...
      getUserMedia(constraints, function (stream) {
        if (constraints.video.mandatory && constraints.video.mandatory.chromeMediaSource === 'desktop') {
          // we have the video stream, but no audio
          // run getUserMedia again to get an audio stream
          getUserMedia({ video: false, audio: true }, function(aStream) {
            videoStream = stream;
            audioStream = aStream;
            start();
          }, callback);
